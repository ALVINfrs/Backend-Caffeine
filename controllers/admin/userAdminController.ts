...
export const getAllUsers = async (req: Request, res: Response) => {
  ...
  try {
    const [users] = await db.query(
      "SELECT id, name, email, phone, role, created_at FROM users LIMIT ? OFFSET ?",
      [limit, offset]
    );
  ...
  try {
    ...
    if (error.code === "ER_ROW_IS_REFERENCED_2") {
      return res
        ...
        .json({
          message:
            "Tidak dapat menghapus pengguna ini karena masih memiliki data terkait (pesanan/reservasi).",
        });
    }
    res.status(500).json({ message: "Internal Server Error" });
  }
