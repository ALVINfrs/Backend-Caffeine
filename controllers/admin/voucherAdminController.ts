...
export const createVoucher = async (req: Request, res: Response) => {
    const { code, name, description, discount_type, discount_value, min_order, max_discount, is_active } = req.body;

    try {
        const [result]: any = await db.query(
            'INSERT INTO vouchers (code, name, description, discount_type, discount_value, min_order, max_discount, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [code, name, description, discount_type, discount_value, min_order, max_discount, is_active || 1]
        );
...
export const updateVoucher = async (req: Request, res: Response) => {
    ...
    try {
        const [result]: any = await db.query(
            ...
            [code, name, description, discount_type, discount_value, min_order, max_discount, is_active, id]
        );
...
export const deleteVoucher = async (req: Request, res: Response) => {
    try {
        const [result]: any = await db.query('DELETE FROM vouchers WHERE id = ?', [req.params.id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Voucher tidak ditemukan' });
        }
        res.status(204).send();
