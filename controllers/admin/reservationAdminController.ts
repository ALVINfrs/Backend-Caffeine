...
export const getAllReservations = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [[{ total }]] : any = await db.query(countQuery, status ? [status] : []);
...
export const updateReservation = async (req: Request, res: Response) => {
    const { id } = req.params;
    const { status, customer_name, phone, reservation_date, reservation_time, guest_count, table_number } = req.body;
    ...
    try {
        const [result]: any = await db.query(
            ...
            [status, customer_name, phone, reservation_date, reservation_time, guest_count, table_number, id]
        );
...
export const deleteReservation = async (req: Request, res: Response) => {
    try {
        const [result]: any = await db.query('DELETE FROM reservations WHERE id = ?', [req.params.id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Reservasi tidak ditemukan' });
        }
        res.status(204).send();
