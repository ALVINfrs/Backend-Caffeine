...
async function createInitialOrder(connection: PoolConnection, orderData: OrderData, items: OrderItem[]): Promise<{ orderId: number, orderNumber: string }> {
    try {
        const {
            userId,
            customerName,
...
async function updateOrderStatus(
    ...
    vaNumber: string | null = null,
    bank: string | null = null,
    fraudStatus: string | null = null,
...
async function getOrderById(orderId: string): Promise<{ order: Order, items: OrderItem[] } | null> {
    ...
    if (order.voucher_id) {
        voucherInfo = {
            id: order.voucher_id,
            code: order.voucher_code as string,
            name: (order as any).voucher_name,
            description: (order as any).voucher_description,
            discountType: (order as any).voucher_discount_type,
            discountValue: (order as any).voucher_discount_value,
...
async function getOrderByNumber(orderNumber: string): Promise<{ order: Order, items: OrderItem[] } | null> {
    const [orders] = await pool.query<Order[]>(
        `SELECT o.*, 
            v.name as voucher_name,
            v.description as voucher_description,
